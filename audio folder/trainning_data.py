...
epod_max = 25000 #Maximun number of Training round
epod = 0 #start Epod
## initialize the Input Layer:
original_label = np.asarray([0]*len(input1) + [1]*len(input2)+ [2]*len(input3)+ [3]*len(input4)+ [4]*len(input5)+ [5]*len(input6)+ [6]*len(input7)+ [7]*len(input8)).T
x = np.concatenate((input1,input2,input3,input4,input5,input6,input7,input8), axis = 0).T
...
while (E>E_stop) and (epod<epod_max): ## initialize the Stopping Condition
  ...
  for i in range(x.shape[1]):
    ...
    dE_dw = np.asarray(dE_dw).reshape(Class,1,21)
    ...
    E = E + 0.5*((e[0])**2 + (e[1])**2 + (e[2])**2 +(e[3])**2 + (e[4])**2 + (e[5])**2 + (e[6])**2 +(e[7])**2) 
...
plt.plot(lam_store)
plt.grid(True) 
savemat('model_saved\weight_8_class.mat', mdict={'w': w})
savemat('model_saved\Lambda_8_class.mat', mdict={'lambda': Lambda})
