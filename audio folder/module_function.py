...
def LGU_network(xi,w,Lambda):
    ...
    ah1 = sigmoid(net[0],Lambda)
    ah2 = sigmoid(net[1],Lambda)
    ah3 = sigmoid(net[2],Lambda)
    ah4 = sigmoid(net[3],Lambda)
    ah5 = sigmoid(net[4],Lambda)
    ah6 = sigmoid(net[5],Lambda)
    ah7 = sigmoid(net[6],Lambda)
    ah8 = sigmoid(net[7],Lambda)
    ah = np.asarray([ah1,ah2,ah3,ah4,ah5,ah6,ah7,ah8])
    ## Softmax Function: 
...
def evaluate(mfcc_feat,w,Lambda,label,Bias =1):
    ...
    print(result)
    print(z)
    answers = label[result[0]]
    percent = np.round(z[result[0]][0],2)*100
    print('your answers is '+ answers[0] +': {0} %'.format(percent[0][0]))
    return  answers[0],percent[0][0]

def record(Weigth,Lambda,label,filename = "record.wav",seconds=1.5):
